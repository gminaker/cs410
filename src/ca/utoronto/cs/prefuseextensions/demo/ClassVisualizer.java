 ...
 * 
 * Generalized fisheye filter applied, revealing only nodes of depth 1 from
 * focus (root), selected nodes (gold), and search results (orange). Select
 * nodes with a single click. Select multiple with ctrl-click. Deselect with
 * ctrl-click on a selected node. Recenter graph by double clicking any node;
 * new spanning tree is calculated and the graph will be shown centered at that
 * node.
 * 
 * There are some strange rendering effects, but these are documented Java2D
 * bugs with Arc2D rendering (see notes in Sector2D class).
 * 
 * @version 1.2
 * @author Gordon Minaker through modifications of original code by Christopher
 *         Collins through modification of original code by jeffrey heer
 * 
 *         Template from StarburstDemo.java was modified by Gordon Minaker (GM)
 *         in October 2014.
 * 
 *         Mock data objects are represented in a demo() method on line 325.
 * 
 */
public class ClassVisualizer extends Display {
	...
	// CLASS DATASET:
	public static final String DATA_FILE = "/class-data.xml";
	...
	public ClassVisualizer(Graph g, String label) {
		...
		// set location and turn off autoscale so graph layout doesn't revert to
		// original view when mouse wheel is rolled
		...
		// add decorators (has to be after layout because decorators rendered
		// rely on Schema provided by StarburstLayout
		m_vis.addDecorators(labels, treeNodes, new OrPredicate(
				new VisiblePredicate(), new StartVisiblePredicate()),
				ArcLabelRenderer.LABEL_SCHEMA);
		...
		searchAndSelect.addSet(Visualization.SELECTED_ITEMS,
				m_vis.getFocusGroup(Visualization.SELECTED_ITEMS));
		searchAndSelect.addSet(Visualization.SEARCH_ITEMS,
				m_vis.getFocusGroup(Visualization.SEARCH_ITEMS));
		...
		ColorAction nodeStrokeColor = new ColorAction(treeNodes,
		...
		resizeAction = new Action() {
			public void run(double frac) {
				// animate reset zoom to fit the data (must run only AFTER
				// layout)
				Rectangle2D bounds = m_vis.getBounds(tree);

				if (bounds.getWidth() == 0)
					return;
				GraphicsLib.expand(bounds, 10 + (int) (1 / m_vis.getDisplay(0)
						.getScale()));
				DisplayLib.fitViewToBounds(m_vis.getDisplay(0), bounds,
						(long) 1250);
			}
		};
		...
		ActionList repaint = new ActionList() {
			public void run(double frac) {
				// only repaint if animation is not already running; otherwise
				// we get flicker if repaint
				// is called from HoverActionControl while a visibility
		...
		addControlListener(new FocusControl(Visualization.SELECTED_ITEMS, 1,
				"filter"));
	...
	public static void main(String argv[]) {
		...
		// modified by GM
		JFrame frame = new JFrame("Class Visualizer");
	...
	// Written by GM:
	// Inspired by SO:
	// http://stackoverflow.com/questions/7233956/creating-data-in-prefuse
	...
	// GM says no need for this anymore:
	public static JPanel demo(String datafile, final String label) {
		Graph g = null;
		// Can accept either a graph or a tree; try reading a graph first, if it
		// fails, try tree
		try {
	...
	public static JPanel demo(Graph g, final String label) {
		...
		/*
		 * Search panel not needed: // create a search panel for the tree map
		 * SearchQueryBinding sq = new SearchQueryBinding( (Table)
		 * vis.getGroup(treeNodes), label, (SearchTupleSet)
		 * vis.getGroup(Visualization.SEARCH_ITEMS)); JSearchPanel search =
		 * sq.createSearchPanel(); search.setShowResultCount(true);
		...
		/*
		 * No need JValueSlider filterLevelSlider = new
		 * JValueSlider("Filter level", 1, 20, 1);
		 * filterLevelSlider.addChangeListener(new ChangeListener() { public
		 * void stateChanged(ChangeEvent e) {
		 * gview.setFilterLevel(((JValueSlider) e.getSource()).getValue()
		 * .intValue()); } });
		 */
	...
	public static class NodeColorAction extends ColorAction {
		public NodeColorAction(String group) {
			...
			add("_hover and ingroup('" + Visualization.SELECTED_ITEMS + "')",
					ColorLib.brighter(ColorLib.rgb(0, 190, 204)));
			// search results
			add("_hover and ingroup('_search_')",
					ColorLib.brighter(ColorLib.rgb(152, 255, 92)));
			add("_hover", ColorLib.brighter(ColorLib.rgb(61, 130, 246)));
			// selected subtrees
			add("ingroup('" + Visualization.SELECTED_ITEMS + "')",
					ColorLib.rgb(0, 190, 204));
			// search results
	...
	private DefaultRendererFactory createRenderers() {
		...
		DecoratorLabelRenderer decoratorLabelRenderer = new DecoratorLabelRenderer(
				m_label, false, 2);
		...
		rf.add("ingroup('labels') and rotation == 0", arcLabelRenderer); // all
																			// sector
																			// labels
																			// that
																			// are
																			// not
																			// rotated
		rf.add("ingroup('labels') and rotation != 0", decoratorLabelRenderer); // all
																				// rotated
																				// sector
																				// labels
